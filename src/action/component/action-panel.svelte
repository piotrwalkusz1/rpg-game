...
>
  <div class="p-[10px]">
    ...
    {#if !actionContext.isActionRequired}
      <button on:click={actionExecutionContext.doNotTakeAction} class="float-right">X</button>
    {/if}
  </div>
  ...
  <div class="p-[10px]">
    {#each actionContext.actions as action}
      <div on:click={() => action.executeAction(actionExecutionContext)} class="cursor-pointer text-blue-700 hover:text-blue-400">
